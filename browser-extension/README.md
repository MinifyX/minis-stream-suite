...
Ersetzt **in deinem Kanal** die Nachrichtenliste von Twitch durch einen eigenen Chat:
...
```bash
npm run build:extension -- --channel deinkanal
```
...
- `content.js` läuft auf twitch.tv. Im eingestellten Kanal versteckt es die Twitch-Nachrichtenliste und hängt einen eigenen Chat (in einem Shadow DOM) davor ein.
